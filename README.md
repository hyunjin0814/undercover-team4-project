...
| **이동 애니메이션 동기화** — 두 클라이언트 화면. 애니메이션 파라미터를 전혀 보내지 않고 각 피어가 position 변화량으로 역산한다 | **상호작용 아웃라인** — 조준한 대상만 켜진다. 대상 선정과 가림 판정을 분리한 결과 |
...
<img src="docs/images/ragdoll-sync.gif" width="426">

*좌·우는 서로 다른 클라이언트 화면. 호스트가 계산한 자세를 받아 양쪽이 같은 모양으로 무너지고 같은 자리에 정착한다.*
